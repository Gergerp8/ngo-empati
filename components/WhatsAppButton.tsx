...
/** Floating WhatsApp contact button (fixed, safe-area aware). */
export async function WhatsAppButton() {
  const s = await getSettings();
  return (
    <a
      href={`https://wa.me/${s.whatsapp}?text=${encodeURIComponent(
        "Hai NGO Empati, saya ingin bertanya tentang...",
      )}`}
      target="_blank"
      rel="noreferrer"
      aria-label="Hubungi kami di WhatsApp"
      className="fixed bottom-5 right-5 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lift transition-transform hover:scale-105 active:scale-95"
